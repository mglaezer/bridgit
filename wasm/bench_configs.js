...
async function main() {
  ...
  var setDepth = bluMod.cwrap('wasm_set_depth', null, ['number']);
  var setBaseWidths = bluMod.cwrap('wasm_set_base_widths', null, ['number', 'number', 'number', 'number']);
  var setWidths = bluMod.cwrap('wasm_set_widths', null, ['number', 'number']);
  ...
  var configs = [
    { name: 'w1000/1000 d6 [61,20,20,10]+[8,6]', red_w: 1000, blue_w: 1000, depth: 6, base: [61,20,20,10], extra: [8,6] },
    { name: 'w2000/1000 d6 [61,20,20,10]+[8,6] (current)', red_w: 2000, blue_w: 1000, depth: 6, base: [61,20,20,10], extra: [8,6] },
    { name: 'w3000/1000 d6 [61,20,20,10]+[8,6]', red_w: 3000, blue_w: 1000, depth: 6, base: [61,20,20,10], extra: [8,6] },
    { name: 'w2000/1000 d6 [61,30,30,15]+[10,8]', red_w: 2000, blue_w: 1000, depth: 6, base: [61,30,30,15], extra: [10,8] },
    { name: 'w2000/1000 d8 [61,20,14,10]+[8,6]+[4,3]', red_w: 2000, blue_w: 1000, depth: 8, base: [61,20,14,10], extra: [8,6] },
  ];
  ...
  for (var ci = 0; ci < configs.length; ci++) {
    ...
    setDepth(cfg.depth);
    setBaseWidths(cfg.base[0], cfg.base[1], cfg.base[2], cfg.base[3]);
    setWidths(cfg.extra[0], cfg.extra[1]);
    var wins = 0;
    var t0 = Date.now();
    ...
    console.log(cfg.name + ': ' + wins + '/' + NUM + ' (' + (100*wins/NUM).toFixed(1) + '%) ' + (elapsed/1000).toFixed(1) + 's');
  }
}
