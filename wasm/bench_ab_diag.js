...
async function main() {
  ...
  var getLastDepth = mod.cwrap('wasm_get_last_depth', 'number', []);
  var getLastNodes = mod.cwrap('wasm_get_last_nodes', 'number', []);
  ...
  console.log('Move | Depth | Nodes   | Time(ms) | Unclaimed');
  console.log('-----|-------|---------|----------|----------');
  ...
  for (var turn = 0; turn < 122; turn++) {
    ...
    if (game.turn === 'red') {
      ...
      var dt = Date.now() - t0;
      var depth = getLastDepth();
      var nodes = getLastNodes();

      console.log(
        String(moveNum).padStart(4) + ' | ' +
        String(depth).padStart(5) + ' | ' +
        String(nodes).padStart(7) + ' | ' +
        String(dt).padStart(8) + ' | ' +
        String(unclaimed.length).padStart(9)
      );
