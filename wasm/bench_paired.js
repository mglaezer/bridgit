...
async function main() {
  ...
  var wasmDepth = parseInt(process.argv[3]) || 6;
  wasmSetDepth(wasmDepth);

  console.log('Paired benchmark: JS 4-ply vs WASM ' + wasmDepth + '-ply (' + NUM_GAMES + ' games vs weakRed-0.9)');

  var jsWins = 0, wasmWins = 0, both = 0, neither = 0;
  var jsOnly = 0, wasmOnly = 0;
  ...
  console.log('JS 4-ply:     ' + jsWins + '/' + NUM_GAMES + ' (' + (100 * jsWins / NUM_GAMES).toFixed(1) + '%)');
  console.log('WASM ' + wasmDepth + '-ply:  ' + wasmWins + '/' + NUM_GAMES + ' (' + (100 * wasmWins / NUM_GAMES).toFixed(1) + '%)');
